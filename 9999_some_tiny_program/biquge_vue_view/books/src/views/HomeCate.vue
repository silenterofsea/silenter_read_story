        ...
        <b-col id="main-left" cols="12" md="7">
          ...
          >
            ...
            <thead role="rowgroup">
              ...
              <tr role="row" class="">
                <th
                  v-for="(item, index) in newestFields"
                  :key="index"
                  role="columnheader"
                  scope="col"
                  :aria-colindex="index"
            ...
            <tbody role="rowgroup">
              <!---->
              <tr
                v-for="(item, index) in cateItems.newestItems"
                :key="index"
                role="row"
              >
                <!-- <td v-for="(itemTd, indexTd) in item" :key="indexTd" :aria-colindex="indexTd" role="cell" >{{ itemTd }}</td> -->
                <td :aria-colindex="item" role="cell">
                  <a :href="'/book/' + item.book_id" class="success">{{
                    item.book_name
                  }}</a>
                </td>
                <td :aria-colindex="item" role="cell">
        ...
        <b-col id="main-right" cols="12" md="4">
          ...
          >
            ...
            <thead role="rowgroup">
              ...
              <tr role="row" class="">
                <th
                  v-for="(item, index) in mostFields"
                  :key="index"
                  role="columnheader"
                  scope="col"
                  :aria-colindex="index"
            ...
            <tbody role="rowgroup">
              <!---->
              <tr
                v-for="(item, index) in cateItems.mostItems"
                :key="index"
                role="row"
              >
                <td :aria-colindex="item" role="cell">
...
export default {
  ...
  setup(props, context) {
    const newestFields = reactive({
      book_name: "小说",
      book_newest_name: "最新章节",
      book_last_update_time: "更新时间"
    });
    ...
    GetInfoPOST(contentParams).then(resp => {
      ...
      document
        ...
        .setAttribute("content", resp.data.data.keywords);
      document
        .querySelector('meta[name="description"]')
        .setAttribute("content", resp.data.data.description);
    });
    // 设置关键词结束
